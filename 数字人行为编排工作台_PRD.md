...
平台采用 7 个一级 Tab。Runtime States 属于底层系统链路，收纳到 Base 二级入口；Business / Intent / Proactive 统一收纳到 Strategies。

| Tab | 名称（UI 英文） | 核心职责 |
| --- | --- | --- |
| Tab 1 | Global Base | 管理 System Core、Guardrails、Runtime States |
| Tab 2 | Personas | 独立维护可复用 Persona Prompt、版本和状态 |
| Tab 3 | Idle Motions | 独立维护可复用静默动作、Recommended Chunk Count 和动作 Prompt |
| Tab 4 | Character Center | 管理视觉、背景、声音、TTS Link、Boundaries，并选择 Persona / Idle Motions |
| Tab 5 | Strategies | 管理 Session Scenario Base、用户意图策略、主动策略、匹配样例与阈值 |
| Tab 6 | Sandbox Simulation | 展开 Full Prompt，进行仿真、热力图和 Token 检查 |
| Tab 7 | Online Traceability | 查看线上 Turn 级组装记录、分层 Prompt、模型输出 |
...
统一交互规则：所有 Admin Table 的 **Edit** 操作均在右侧 Drawer 打开，保留原列表上下文；Run / View 详情不受此规则限制。
...
|    |  - System Core              |                                                          |
|    |  - Runtime States           |  - 列表：Search + New + Table                            |
...
| Tab | 二级资产栏 | 主工作区模式 |
| --- | --- | --- |
| Tab 1 Global Base | 有（System Core / Runtime States） | Core 编辑 / States + Transitions |
| Tab 2 Personas | 无 | 列表 → Edit Persona Prompt 详情 |
| Tab 3 Idle Motions | 无 | 列表 → Edit Idle Motion 详情 |
| Tab 4 Character Center | 无 | 列表 → Edit Avatar 详情 |
| Tab 5 Strategies | 无 | Business / Proactive 子视图 → Edit 详情 |
| Tab 6 Sandbox Simulation | 无 | 列表 → Run 详情 |
| Tab 7 Online Traceability | 无 | 列表 → View 详情 |
...
```text
+-----------------------------------------------------------------------------------------------+
|    | [Avatar: Jennie v2.3] [环境: Dev | Staging | Prod] [版本状态] [保存草稿] [合并/发布工单] |
+----+------------------------------------------------------------------------------------------+
...
全局基底负责维护系统级 Prompt、工具编排规则、模型路由配置与底层运行状态机。

该 Tab 二级入口：

- System Core：算法铁律、模型路由、工具调用策略、输出契约、运行默认值、可观测性规则。
- Runtime States：Idle / Proactive / Interaction / Fallback 与 Transitions。
...
- Algorithm Iron Rules：动作能力协议、输出格式约束、合规 Guardrails、异常兜底策略。
- Model Routing：LLM、生图、试衣、视频生成、TTS 的默认模型与 fallback 模型。
- Tool Calling Policy：何时调用 try-on、image generation、video generation、TTS。
- Output Contract：结构化输出字段、action token、look_id、tool_calls、tts_text 等契约。
- Runtime Defaults：默认语言、超时、冷却、重试、tail reinforcement、fallback 策略。
- Observability：Trace span、脱敏、记录字段和线上排查粒度。
...
### 5.3 Tab 5：业务策略仓库 Biz Strategies
...
Tab 1 与 Tab 5 交互（MVP 已实现）：

- Base 左侧二级栏包含多个系统模块入口：`Algorithm Iron Rules`、`Image Generation Prompts`、`Fallback Prompts`、`User Context Templates`、`Model Routing`、`Tool Calling Policy`、`Output Contract`、`Runtime Defaults`、`Observability`、`Runtime States`。
- **System Base 模块**：每个入口下展示该模块需要的 Prompt 文件列表，列包含 Name、Versions、Type、Latest Version Created At、Number of Observations、Actions。
- 每条 Prompt 文件提供 `Edit` 按钮；Edit 以右侧栏打开具体 Prompt 内容和基础元信息。System Base 列表不展示 Prompt 预览、Tags 或 Variables。
- **Business Strategies**：独立一级 Tab；策略包表格 → Edit 进入触发方式、匹配样例、阈值和 Prompt Blocks。
- Type、Version 使用下拉选择，非自由文本输入。

---

## 6. Tab 2 Personas、Tab 3 Idle Motions 与 Tab 4 Character Center

### 6.1 模块定位

Personas 独立维护身份、人格、说话风格和用户关系；Idle Motions 独立维护静默动作资产；Character Center 管理完整 Avatar Profile，并选择已发布 Persona 与多个 Idle Motions。

Persona 不承载试衣任务流程、动作执行、主动触达或意图分类规则。这些内容分别归属 Scenario Base、Business Strategy、Proactive Strategy。
...
- 标题下方展示 Avatar 主预览图。
- 分区：Basic Info、Voice & TTS、Visual Asset、Visual Description、Background Description、Persona Prompt 选择、Idle Motions 多选、Boundaries。
- Persona Prompt 仅下拉选择和只读预览，正文在独立 Personas Tab 维护。
- TTS Link 默认为空，只交给语音合成阶段，不进入 LLM Prompt。
- Idle Motion 通过 `idleMotionIds[]` 多选；只有 Published 动作会进入 Idle 状态的 `{{ idle_motion }}`。
...
Persona 定义角色稳定的身份、人格、说话风格和关系，不定义具体业务任务。
...
- Speech Style。
- Relationship。
- Persona Summary：`styleSummary`、`relationshipSummary`、`speakingStyleTags`、`forbiddenStyleTags`，用于 Proactive / Fallback 等轻量拼接场景。

明确不放入 Persona：

- 试衣业务目标和体验流程（归入 Session Scenario Base）。
- 换装、转身、评价、闲聊等意图处理（归入 Intent Strategy）。
- 无输入后的主动话术（归入 Proactive Strategy）。
- 动作与合规硬约束（归入 System / Boundaries）。

示例：

```text
You are Jennie, a stylish and approachable live-stream host who helps viewers
...
```text
States 决定：当前 state、可参与匹配的 strategyIds、User Context 模板、Interrupt Buffer
        ↓
组装引擎按当前状态选择性拼接：System → Character →（Persona / Scenario / Strategy 按状态与意图注入）→ User Context
        ↓
Tab 6 沙箱 / Tab 7 线上追溯记录完整结果
```

MVP **不采用 DAG 画布**，而通过两张 Admin Table 管理：

- **States**：状态节点及其策略绑定、User Context、打断缓冲策略。
...
| Context Variables / User Context Template | 按状态类型 context-aware 展示（见 7.2.1） |
| Intent Strategy Candidates | 可参与本状态语义匹配的意图策略候选 |
| Attached Proactive Strategies | 主动策略多选（Tab 6） |
| Interrupt Buffer Lock | 下拉：Disabled / Policy A Hard Cut / Policy B 1.5s Fade Buffer |

#### 7.2.1 Context-aware 字段显隐

| 状态类型 | User Context | 业务策略包 | 主动策略 |
| --- | --- | --- | --- |
| Idle Standby | 仅展示变量占位符（如 `idle_motion`），系统自动包裹 | **禁用** | **禁用** |
| Proactive Active | 可编辑模板 + 变量 pill | 禁用（Scenario Base 仍常驻） | **可绑定** |
...
- 节点类型。
- 默认描述。
- Session 级常驻策略由状态机 `scenarioStrategyIds` 统一绑定。
- 节点 `strategyIds` 仅声明可参与该状态语义匹配的意图策略候选。
- 绑定主动策略 `proactiveStrategyIds`。
- Interrupt Buffer Policy。
- User Context 模板或变量声明。
...
- **Tab 5 Strategies / Proactive 子视图**：定义进入 Proactive 等状态后 *说什么、做什么*。
...
当用户在静默、主动、展示等状态中突然插话，状态机需要读取上一轮 History Buffer 中的动作状态，并注入本轮 User Context。
...
用户突然插话："这件衣服多少钱？"
装配引擎读取上一轮动作 Buffer
User Context 注入：请先自然收尾当前微表情，再进入价格讲解
```
...
Business Strategy 分为两类：

- `activationMode=session`：进入业务 Session 后始终注入 **Scenario Base** 层。
- `activationMode=intent`：用户输入后，只在候选策略的语义相似度达到阈值时注入 **Business Strategy** 层；只取最高分命中项。

数据来源：

- Tab 5 Business Strategies 中已启用的策略包。
- 当前 Avatar / 环境可用的策略包。

Idle / Proactive 等非交互态不执行意图匹配。Proactive 由独立主动策略根据 timeout、cooldown、priority 触发，不依赖用户输入相似度。
...
+ Idle_Motion
+ Idle_Dynamic_Context

Proactive =
System_Action_Rules
+ Avatar_Profile_Light（含 personaPromptId 与 persona style summary）
+ Proactive_Strategy
+ Proactive_Dynamic_Context

Interaction =
System_Action_Rules
+ Avatar_Profile_Context（按意图 full / light）
+ Persona_Context（仅需要自然表达时）
+ Session_Scenario_Base（仅命中业务意图时）
+ Matched_Intent_Strategy
+ Tool_Prompt（如需要）
+ Interaction_Dynamic_Context

Fallback =
System_Action_Rules
+ Avatar_Profile_Light（含 personaPromptId 与 basic boundaries）
+ Fallback_Prompt
+ Fallback_Dynamic_Context
```
...
- Idle 不注入完整 Persona，也不注入 Scenario Base；仅注入视觉身份、静默动作和 Idle User Context。
- Proactive 不注入完整 Persona 或 Scenario Base；使用 Avatar Profile Light 中的 persona 摘要和 Proactive Strategy。
- Session Scenario Base 仅在 Interaction 命中业务意图时注入；Intent Strategy 仅在用户输入命中相似度阈值后注入。
- Fallback 使用 Avatar Profile Light + Fallback Prompt，避免长 Persona 稀释安全规则。
- Persona 不包含 Scenario 或 Intent Strategy 内容。
...
## 9. Tab 5：Strategies / Proactive 子视图

### 9.1 模块定位

主动策略用于管理数字人在 **已进入特定状态后** 的主动表达（说什么、做什么），**不负责**定义状态流转时机。

与 Base → Runtime States 的分工：

| 模块 | 职责 |
| --- | --- |
| Base → Runtime States / Transitions | 何时从 Idle → Proactive Active（如 15s 无输入） |
| Tab 5 Strategies / Proactive 子视图 | 进入 Proactive Active 后播放哪条话术、哪个动作 |
...
- 选择状态节点。
- 输入用户 Query。
- 展示预期 Intent 和预期 Tool Calls。
- 展开本轮实际命中的 Full Prompt。
- 查看实际命中的分层 Prompt。
- 查看 Tool Call Plan。
- 查看结构化 Output Contract。
- 查看 Token 统计。
- 查看热力图。
- 一键复制。
- 保存为测试样例。
...
展示区域按本轮实际命中的装配层级分块，不展示未命中的 prompt 资产：

- System Prompt（如 Action Space、Guardrails、Output Contract）。
- Avatar Profile。
- Persona。
- Scenario Base。
- Business Strategy（仅意图匹配达阈值时）。
- Tool Prompt（仅需要工具调用时，例如 try-on image prompt）。
- Fallback Prompt（仅 Fallback 状态）。
- Tool Call Plan。
- Proactive Strategy（仅 Proactive 状态）。
- User Context。
- Final Full Prompt。
...
线上追溯（UI：**Tracing**）用于还原历史真实 Prompt 拼接现场，采用 LangSmith 风格 **Traces / Observations 双 Tab + 左侧筛选栏 + Span 树详情** 布局。

### 11.2 查询能力（MVP 已实现）

**Traces Tab**（一行 = 一个 Turn）：

- 左侧筛选栏：Environment、Trace Name、Session ID、User ID、Avatar、State Node（多选 checkbox）。
- 顶栏：Hide filters、Search（All / IDs / Names）、Time range、Refresh、Columns 显隐、Export。
- 表格列：Select、Bookmark、Timestamp、Name、State、Strategy Decision、Input、Output、Observation Levels。
- 页面顶部提供 Idle / Proactive / Interaction 三张说明卡。
- Demo Trace 覆盖：Idle 无任务策略、Proactive 命中主动策略、Interaction 命中意图策略、Interaction 未达到阈值而无策略。

**Observations Tab**（扁平 Span 列表）：

- 列：Timestamp、Trace Name、Span Path、Kind、Duration、Input、Output。
- 点击行跳转对应 Trace 详情并定位到该 Span。
...
### 11.3 快照内容与 Span 树（MVP 已实现）

每条 Turn 快照包含 **Turn Assembly Record**，与沙箱 `renderPrompt()` 逻辑一致。UI 层从 `TraceSnapshot` 派生 Span 树：
...
| `promptLayers[]` | 分层 Prompt（System / Avatar / Persona / Strategy / User Context） |
| `dynamicContextRendered` | 渲染后的 User Context 文本 |
| `fullPrompt` | 完整 Final Prompt |
| `totalTokens` | Token 统计 |

**详情页（Span Tree + Preview）**：

- 左侧：可折叠 Span 树，展示 name + duration。
- 右侧 Preview Tab：Input / Output 的 Path-Value 树或 JSON 视图；Formatted / JSON 切换。
- 顶栏 Metadata：assembly latency、model latency、User ID（脱敏）、Environment、Tokens。
- LangSmith 协作按钮（Add to datasets / Annotate / Add comment）：MVP 占位，Coming soon。
- Log View / Scores Tab：MVP 占位。
- Corrected Output (Beta)：MVP 占位。
- **View Full Snapshot**：切换脱敏 / 完整 User ID、userInput、fullPrompt、modelOutput。
...
- **Tab 1 Global Base**：System Core + Runtime States（高级系统配置）。
- **Tab 2 Character Center**：Avatar 列表 → Edit Avatar；单 Card 预览图；Persona / Script Library。
- **Tab 5 Strategies**：Session / Intent / Proactive 策略列表 → Edit；匹配样例、阈值、Bound State、Cooldown、Text Atom 与 Emotion-Action 校验。
- **Tab 6 Sandbox Simulation**：Run 列表 → 详情；Full Prompt 分层展示。
